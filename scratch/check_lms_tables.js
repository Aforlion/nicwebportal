...
async function run() {
  const tables = ['courses', 'modules', 'lessons', 'cpd_activities', 'cpd_records', 'course_enrollments', 'enrollments'];
  for (const t of tables) {
    const { data, error } = await supabase.from(t).select('*').limit(1);
    console.log(`Table '${t}':`, error ? error.message : (data?.length ? Object.keys(data[0]) : 'empty table'));
  }
}
